...
public class EnchantmentUtils {
   public static boolean hasConflictingEnchanment(ItemStack item, Enchantment ench) {
      ...
      Iterator var4 = enchantments.keySet().iterator();

      while(var4.hasNext()) {
         Enchantment e = (Enchantment)var4.next();
         if (ench.conflictsWith(e)) {
            conflict = true;
         }
      }

      if (!enchantments.containsKey(ench) && !conflict) {
         return false;
      } else {
         return true;
      }
   ...
   public static Enchantment getEnchantmentFromCommonName(String name) {
      if (name.equalsIgnoreCase("aquaaffinity")) {
         ...
         return name.equalsIgnoreCase("unbreaking") ? Enchantment.DURABILITY : null;
      }
   }
