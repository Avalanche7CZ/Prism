...
public final class LinkedTreeMap extends AbstractMap implements Serializable {
   ...
   public Object get(Object key) {
      Node node = this.findByObject(key);
   ...
   public Object put(Object key, Object value) {
      if (key == null) {
         ...
         Node created = this.find(key, true);
   ...
   public Object remove(Object key) {
      Node node = this.removeInternalByKey(key);
      return node != null ? node.value : null;
   }
   ...
   Node findByEntry(Map.Entry entry) {
      Node mine = this.findByObject(entry.getKey());
   ...
   static final class Node implements Map.Entry {
      ...
      Node(Node parent, Object key, Node next, Node prev) {
         this.parent = parent;
         this.key = key;
         this.height = 1;
         this.next = next;
      ...
      public Node first() {
         ...
         for(Node child = this.left; child != null; child = child.left) {
            node = child;
         }
      ...
      public Node last() {
         ...
         for(Node child = this.right; child != null; child = child.right) {
            node = child;
         }
